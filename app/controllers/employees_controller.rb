class EmployeesController < ApplicationController
  ...
  def index
    if current_user.role?(:manager)
      ...
      @inactive_employees = Employee.inactive.for_store(current_user.store_id).paginate(page: params[:page]).per_page(10)
    else
  ...
  def show
    ...
    @shifts = Shift.upcoming.for_employee(@employee.id).chronological.paginate(page: params[:page]).per_page(5)      
    @user = @employee.user    
  ...
  def employee_params
    params.require(:employee).permit(:first_name, :last_name, :ssn, :date_of_birth, :role, :phone, :active, user_attributes: [:id, :email, :password, :password_confirmation, :employee_id])
  end
